    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            # ====== YOUR CODE: ======
            # 1. הרצת אופוק אימון ואופוק בדיקה
            train_result = self.train_epoch(dl_train, verbose=verbose, **kw)
            test_result = self.test_epoch(dl_test, verbose=verbose, **kw)
            
            actual_num_epochs += 1
            
            epoch_train_loss = sum(train_result.losses) / len(train_result.losses)
            epoch_train_acc = sum(train_result.accuracy) / len(train_result.accuracy) if isinstance(train_result.accuracy, list) else train_result.accuracy
            
            epoch_test_loss = sum(test_result.losses) / len(test_result.losses)
            epoch_test_acc = sum(test_result.accuracy) / len(test_result.accuracy) if isinstance(test_result.accuracy, list) else test_result.accuracy
            ...
            if early_stopping is not None and epochs_without_improvement >= early_stopping:
                self._print(f"\nEarly stopping triggered after {epochs_without_improvement} epochs without improvement.", verbose)
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        loss = self.loss_fn(out.view(-1, self.model.out_dim), y.view(-1))
