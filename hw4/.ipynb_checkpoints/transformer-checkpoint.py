...
def sliding_window_attention(q, k, v, window_size, padding_mask=None):
    ...
    # 2. יצירת מסכת החלון (Window Mask) בגישה אבסולוטית (מתאימה לרוב התרגילים)
    ...
    # החלת המסכה: משתמשים ב--1e9 למנוע NaN
    scores = scores.masked_fill(~window_mask, -1e9)
    
    # 3. החלת מסכת הריפוד (Padding Mask)
    if padding_mask is not None:
        if len(q.shape) == 4:
            # מתאימים למימדים [Batch, Head, SeqLen_q, SeqLen_k]
            pad_mask = (padding_mask == 0).unsqueeze(1).unsqueeze(2) 
        else:
            pad_mask = (padding_mask == 0).unsqueeze(1).unsqueeze(1)
            
        scores = scores.masked_fill(pad_mask.to(q.device), -1e9)
        
    # 4. Softmax + הכפלה
    attention = F.softmax(scores, dim=-1)
    
    # טיפול אקסטרה: איפוס מוחלט של שורות שהיו כולן פאדינג כדי למנוע חלחול של אשפה
    if padding_mask is not None:
        if len(q.shape) == 4:
            attn_mask = padding_mask.unsqueeze(1).unsqueeze(-1)
        else:
            attn_mask = padding_mask.unsqueeze(-1)
        attention = attention * attn_mask.to(q.device)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, padding_mask):
        ...
        # ====== YOUR CODE: ======
        # 1) Apply attention to the input x, and then apply dropout.
        # שומרים עותק נקי של הקלט המקורי בשביל ה-residual
        original_x = x 
        ...
        # 2) Add a residual connection from the original input and normalize.
        out_1 = self.norm1(original_x + attn_out)
        
        # 3) Apply a feed-forward layer to the output of step 2, and then apply dropout again.
        ff_out = self.feed_forward(out_1)
        ff_out = self.dropout(ff_out)
        
        # 4) Add a second residual connection and normalize again.
        out_2 = self.norm2(out_1 + ff_out)
...
class Encoder(nn.Module):
    ...
    def forward(self, sentence, padding_mask):
        ...
        cls_token = x[:, 0, :]  # Shape: [Batch, Dims]
        output = self.classification_mlp(cls_token)
        
        # שים לב: מחקנו מכאן את ה-squeeze! משאירים את התוצאה כ-[Batch, 1] בדיוק כמו שהטסט מצפה
        # ========================
