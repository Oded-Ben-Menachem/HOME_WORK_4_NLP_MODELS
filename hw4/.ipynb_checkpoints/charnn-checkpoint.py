...
def remove_chars(text: str, chars_to_remove):
    ...
    n_removed = orig_len - len(text_clean)   
    # ========================
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
   ...
   # ====== YOUR CODE: ======
    ...
    total_chars_to_use = num_samples * seq_len
    
    samples_text = text[:total_chars_to_use]
    labels_text = text[1 : total_chars_to_use + 1]
    
    samples_onehot = chars_to_onehot(samples_text, char_to_idx)
    V = len(char_to_idx)
    samples = samples_onehot.view(num_samples, seq_len, V).to(device)
    
    label_indices = [char_to_idx[char] for char in labels_text]
    labels = torch.tensor(label_indices, dtype=torch.long).view(num_samples, seq_len).to(device)
    # ========================
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    # ====== YOUR CODE: ======
    with torch.no_grad():
        x_start = chars_to_onehot(start_sequence, char_to_idx).unsqueeze(0).to(dtype=torch.float, device=device)
        
        y, h = model(x_start, hidden_state=None)
        ...
        for _ in range(chars_to_generate):
            ...
            x_next = chars_to_onehot(next_char, char_to_idx).unsqueeze(0).to(dtype=torch.float, device=device)
            
            y, h = model(x_next, hidden_state=h)
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        num_batches = M // B
        total_elements = num_batches * B
        
        matrix = torch.arange(total_elements).view(B, num_batches)
        
        matrix_contiguous = matrix.t()
        
        idx = matrix_contiguous.flatten().tolist()
        # ========================
        return iter(idx)
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for i in range(self.n_layers):
            current_in_dim = self.in_dim if i == 0 else self.h_dim
            
            layer_modules = {
                'w_xz': nn.Linear(current_in_dim, self.h_dim, bias=True),
                'w_hz': nn.Linear(self.h_dim, self.h_dim, bias=False),
                
                'w_xr': nn.Linear(current_in_dim, self.h_dim, bias=True),
                'w_hr': nn.Linear(self.h_dim, self.h_dim, bias=False),
                
                'w_xg': nn.Linear(current_in_dim, self.h_dim, bias=True),
        ...
        self.fc_out = nn.Linear(self.h_dim, self.out_dim, bias=True)
        # ========================
    ...
    def forward(self, input: Tensor, hidden_state: Tensor = None):
        ...
        # ====== YOUR CODE: ======
        outputs = []
        final_hidden_states = [None] * self.n_layers
        ...
        for t in range(seq_len):
            ...
            for i in range(self.n_layers):
                ...
                z = torch.sigmoid(params['w_xz'](current_input) + params['w_hz'](h_prev))
                
                r = torch.sigmoid(params['w_xr'](current_input) + params['w_hr'](h_prev))
                
                g = torch.tanh(params['w_xg'](current_input) + r * params['w_hg'](h_prev))
                
                h_new = z * h_prev + (1 - z) * g
                
                layer_states[i] = h_new
                final_hidden_states[i] = h_new
                
                current_input = h_new
                
                if self.dropout_layer and i < self.n_layers - 1:
                    current_input = self.dropout_layer(current_input)
            
            outputs.append(current_input)
            
        layer_output_rnn = torch.stack(outputs, dim=1)
        
        layer_output = self.fc_out(layer_output_rnn)
        
        hidden_state = torch.stack(final_hidden_states, dim=1)
        # ========================
